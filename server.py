...
class Server(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        #FOR UPLOADING FILES
            ...
            self.send_response(200);
            self.send_header("Content-type", "text/html");
            self.send_header("Content-length", len(file_upload));
            self.end_headers();
            self.wfile.write(bytes(file_upload, "utf-8"));
        ...
        #FOR REMOVING ELEMENTS
            ...
            if(works == True):
                try:
                    self.db.removeElement(int(postvars["'removeCode"][0]));
                except:
                    print("failed");
            else:
                print("invalid")
